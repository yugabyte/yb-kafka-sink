...
import com.yugabyte.jdbc.dialect.DatabaseDialect.ColumnConverter;
import com.yugabyte.jdbc.source.ColumnMapping;
import com.yugabyte.jdbc.util.ColumnDefinition;
import com.yugabyte.jdbc.util.ColumnId;
 ...
 * function.
 */
public final class CDCSchemaMapping {
   ...
   */
  public static CDCSchemaMapping create(
      String schemaName, Common.SchemaPB metadata, DatabaseDialect dialect) throws SQLException {

    Map<ColumnId, ColumnDefinition> colDefns = describeColumns(metadata);
    ...
    return new CDCSchemaMapping(schema, colConvertersByFieldName);
  }

  private static Map<ColumnId, ColumnDefinition> describeColumns(Common.SchemaPB metadata) {
    Map<ColumnId, ColumnDefinition> result = new LinkedHashMap<>();

    /*for (int i = 1; i <= metadata.getColumnsCount(); ++i) {
      ColumnDefinition defn = describeColumn(metadata, i);
  ...
  private CDCSchemaMapping(Schema schema, Map<String, ColumnConverter> convertersByFieldName) {
    assert schema != null;
  ...
  public static final class FieldSetter {
    ...
    private FieldSetter(ColumnConverter converter, Field field) {
