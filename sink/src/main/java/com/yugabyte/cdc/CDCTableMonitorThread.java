...
import com.yugabyte.jdbc.util.ConnectionProvider;
import com.yugabyte.jdbc.util.QuoteMethod;
 ...
 */
public class CDCTableMonitorThread extends Thread {
  private static final Logger log = LoggerFactory.getLogger(CDCTableMonitorThread.class);
  ...
  public synchronized List<TableId> tables() {
    ...
    if (!duplicates.isEmpty()) {
      ...
      String msg =
          "The connector uses the unqualified table name as the topic name and has "
              ...
              + "configurations. Update the connector's "
              + configText
  ...
  private synchronized boolean updateTables() {
    ...
    if (whitelist != null) {
      for (TableId table : tables) {
        ...
        if (whitelist.contains(fqn1)
            || whitelist.contains(fqn2)
      ...
      for (TableId table : tables) {
        String fqn1 = dialect.expressionBuilder().append(table, QuoteMethod.NEVER).toString();
        String fqn2 = dialect.expressionBuilder().append(table, QuoteMethod.ALWAYS).toString();
        if (!(blacklist.contains(fqn1)
            || blacklist.contains(fqn2)
    ...
    if (!filteredTables.equals(this.tables)) {
      Map<String, List<TableId>> duplicates =
          filteredTables
              .stream()
