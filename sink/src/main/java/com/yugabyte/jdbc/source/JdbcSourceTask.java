...
public class JdbcSourceTask extends SourceTask {
  ...
  public void start(Map<String, String> properties) {
    ...
    for (String tableOrQuery : tablesOrQuery) {
      ...
      if (mode.equals(JdbcSourceTaskConfig.MODE_BULK)) {
        tableQueue.add(new BulkTableQuerier(dialect, queryMode, tableOrQuery, topicPrefix, suffix));
  ...
  protected Map<String, Object> computeInitialOffset(
      String tableOrQuery, Map<String, Object> partitionOffset, TimeZone timezone) {
    if (!(partitionOffset == null)) {
      ...
      if (timestampInitial != null) {
        ...
        log.info(
            "No offsets found for '{}', so using configured timestamp {}",
            tableOrQuery,
            timestampInitial);
      }
  ...
  public List<SourceRecord> poll() throws InterruptedException {
    ...
    Map<TableQuerier, Integer> consecutiveEmptyResults =
        tableQueue.stream().collect(Collectors.toMap(Function.identity(), (q) -> 0));
    while (running.get()) {
  ...
  private void validateNonNullable(
    ...
    try {
      ...
      if ((incrementalMode.equals(JdbcSourceConnectorConfig.MODE_INCREMENTING)
        ...
        throw new ConnectException(
            "Cannot make incremental queries using incrementing column "
                + incrementingColumn
                + " on "
                + table
                + " because this column "
      ...
      if ((incrementalMode.equals(JdbcSourceConnectorConfig.MODE_TIMESTAMP)
        ...
        throw new ConnectException(
            "Cannot make incremental queries using timestamp columns "
                + timestampColumns
                + " on "
                + table
