...
public class OffsetProtocols {
  ...
  /**
   * Provides the partition map for V1 protocol. The table name included is fully qualified and
   * there is also an explicit protocol key.
   *
  ...
  /**
   * Provides the partition map for V0 protocol. The table name included is unqualified and there is
   * no explicit protocol key.
   *
   * @param tableId the tableId that requires partition keys
  ...
  public static Map<String, String> sourcePartitionForProtocolV0(TableId tableId) {
    return Collections.singletonMap(
        JdbcSourceConnectorConstants.TABLE_NAME_KEY, tableId.tableName());
  }
}
