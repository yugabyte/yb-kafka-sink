...
import com.yugabyte.jdbc.util.DateTimeUtils;
 ...
 *
 * <p>At least one of the two columns must be specified (or left as "" for the incrementing column
 * to indicate use of an auto-increment column). If both columns are provided, they are both used to
...
public class TimestampIncrementingTableQuerier extends TableQuerier
  ...
  private void findDefaultAutoIncrementingColumn(Connection db) throws SQLException {
    ...
    if (incrementingColumnName != null && incrementingColumnName.isEmpty()) {
      ...
      for (ColumnDefinition defn :
          dialect
              .describeColumns(
                  db, tableId.catalogName(), tableId.schemaName(), tableId.tableName(), null)
              .values()) {
  ...
  public Timestamp endTimetampValue() throws SQLException {
    final long currentDbTime =
        dialect
            .currentTimeOnDB(stmt.getConnection(), DateTimeUtils.getTimeZoneCalendar(timeZone))
            .getTime();
