...
import com.yugabyte.jdbc.util.TableDefinition;
import com.yugabyte.jdbc.util.TableDefinitions;
import com.yugabyte.jdbc.util.TableId;
import com.yugabyte.jdbc.util.TableType;
...
public class DbStructure {
  ...
  /**
   * Get the definition for the table with the given ID. This returns a cached definition if there
   * is one; otherwise, it reads the definition from the database
   *
   * @param connection the connection that may be used to fetch the table definition if not already
   *     known; may not be null
   ...
   */
  public TableDefinition tableDefinition(Connection connection, TableId tableId)
      throws SQLException {
  ...
  boolean amendIfNecessary(
    ...
    final Set<SinkRecordField> missingFields =
        missingFields(fieldsMetadata.allFields.values(), tableDefn.columnNames());
      ...
      default:
        throw new ConnectException(
            String.format(
                ...
                type.capitalized(), tableId, missingFields, type.jdbcName()));
    }
      ...
      if (!missingField.isOptional() && missingField.defaultValue() == null) {
        throw new ConnectException(
            String.format(
                ...
                type.jdbcName(), tableId, missingField));
      }
    }
    ...
    if (!config.autoEvolve) {
      throw new ConnectException(
          String.format(
              ...
              type.capitalized(), tableId, missingFields));
    }
    ...
    try {
      ...
      if (maxRetries <= 0) {
        throw new ConnectException(
            String.format(
                "Failed to amend %s '%s' to add missing fields: %s", type, tableId, missingFields),
            sqle);
      }
  ...
  Set<SinkRecordField> missingFields(
      Collection<SinkRecordField> fields, Set<String> dbColumnNames) {
    ...
    if (columnNamesLowerCase.size() != dbColumnNames.size()) {
      log.warn(
          "Table has column names that differ only by case. Original columns={}", dbColumnNames);
    }
    ...
    if (missingFieldsIgnoreCase.size() > 0) {
      log.info(
          "Unable to find fields {} among column names {}", missingFieldsIgnoreCase, dbColumnNames);
    }
