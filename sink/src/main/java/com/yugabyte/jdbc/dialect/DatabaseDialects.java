...
import com.yugabyte.jdbc.dialect.DatabaseDialectProvider.JdbcUrlInfo;
...
public class DatabaseDialects {
  ...
  private static final ConcurrentMap<String, DatabaseDialectProvider> REGISTRY =
        ...
        new PrivilegedAction<Void>() {
          public Void run() {
            ServiceLoader<DatabaseDialectProvider> loadedDialects =
                ServiceLoader.load(DatabaseDialectProvider.class);
   ...
   * DatabaseDialectProvider#score(JdbcUrlInfo) score} for the supplied JDBC URL and Connection, and
   * return a new instance of that dialect. Note that the DatabaseDialect needs to be {@link
   * DatabaseDialect#close() closed}.
   *
   ...
   */
  public static DatabaseDialect findBestFor(String jdbcUrl, AbstractConfig config)
  ...
  /**
   * Get the dialect with the specified name. Note that the DatabaseDialect needs to be {@link
   * DatabaseDialect#close() closed}.
   *
   ...
   */
  public static DatabaseDialect create(String dialectName, AbstractConfig config)
      throws ConnectException {
    ...
    throw new ConnectException(
        "Unable to find dialect with name '"
            + dialectName
            + "' in the available dialects: "
            + dialectNames);
  }
