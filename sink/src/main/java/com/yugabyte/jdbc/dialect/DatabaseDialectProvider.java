...
/**
 * An abstract provider of {@link DatabaseDialect} instances, and used to {@link #score(JdbcUrlInfo)
 * score} the dialect for a given JDBC URL.
 *
 ...
 * providers are then consulted any time someone attempts to {@link
 * DatabaseDialects#findBestFor(String, AbstractConfig) find the best dialect} for a given URL and
 * JDBC source or sink connector configuration. The dialect providers compute a score for the URL,
 * and the dialect provider with the highest score is then used to create a new {@link
 * DatabaseDialect} instance.
 *
 * <p>To implement a new dialect, extend this class and place the name of the implementation class
 * in a file that will appear in the JAR at {@code
 * META-INF/services/com.yugabyte.jdbc.dialect.DatabaseDialectProvider}.
 *
 * <p>For example, all of the dialects providers provided by the Confluent JDBC connector are listed
 * in the {@code META-INF/services/com.yugabyte.jdbc.dialect.DatabaseDialectProvider}. Other JARs
 * can be added to the plugin (or classpath) with additional database dialects.
 */
public abstract class DatabaseDialectProvider {
   ...
   * @return the score; may be {@link #NO_MATCH_SCORE} if the dialect does not apply, or greater
   *     than or equal to {@link #NO_MATCH_SCORE} if the DatabaseDialect can be used with this
   *     connection.
   */
  ...
  /**
   * An abstract base class for a provider that will return a {@link
   * DatabaseDialectProvider#EXCELLENT_MATCHING_SCORE} if any of the supplied JDBC subprotocols
   * match the subprotocol from the JDBC connection URL, or {@link
   * DatabaseDialectProvider#NO_MATCH_SCORE} otherwise.
   */
  public abstract static class SubprotocolBasedProvider extends DatabaseDialectProvider {
    ...
    protected SubprotocolBasedProvider(String name, final String... subprotocols) {
      this(name, Arrays.asList(subprotocols));
    }

    protected SubprotocolBasedProvider(String name, final Collection<String> subprotocols) {
      super(name);
