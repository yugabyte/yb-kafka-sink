 ...
 *
 * <h2>Customizing DBMS-specific Behavior</h2>
 *
 * The {@link GenericDatabaseDialect} implementation is intended to be general enough to work with
 * most DBMS sources with common JDBC and SQL types. However, it will likely not support some
 * DBMS-specific features and types.
 *
 ...
 * <h3>Mapping Types</h3>
 *
 * This JDBC Source connector creates a {@link Struct} object for each row found in a result set. It
 * uses this Dialect interface to define how database types are mapped to the Connect {@link Schema}
 * types and how database values are converted into the {@link Field} values added to those {@link
 * Struct} objects.
 *
 ...
 *
 * <ol>
 *   <li>The Dialect must use the column metadata to determine <i>how</i> to convert the values to
 *       the correct type so that the converted values can be added to the {@link Struct}. The
 *       Dialect can do this once for each result set, rather than having to do the logic for every
 *       row.
 *   <li>Although most dialects will create converter functions that merely use one of the ResultSet
 *       methods to get the correct value, implementations can easily create converter functions
 *       that have more complex logic.
 *   <li>If the Dialect can return a null converter when a field cannot be converted and should be
 *       skipped entirely for all rows. This more efficient and explicit than a more conventional
 *       approach that calls the dialect to convert each value of the result set.
 ...
 * automatically register all {@link DatabaseDialectProvider} implementations on the classpath.
 * Don't forget to include in your JAR file a {@code
 * META-INF/services/com.yugabyte.jdbc.dialect.DatabaseDialectProvider} file that contains the
 * fully-qualified name of your implementation class (or one class per line if providing multiple
 * implementations).
 *
...
public interface DatabaseDialect extends ConnectionProvider {
   ...
   */
  PreparedStatement createPreparedStatement(Connection connection, String query)
      throws SQLException;
   ...
   * @throws SQLException if there is an error accessing the result set metadata
   */
  Map<ColumnId, ColumnDefinition> describeColumnsByQuerying(Connection connection, TableId tableId)
      throws SQLException;
   ...
   *     none
   * @param timestampColumns the identifiers of the timestamp column; may be null if there is none
   ...
   *
   * <p>This method is only called by the default implementation of {@link
   * #buildInsertStatement(TableId, Collection, Collection, TableDefinition)}, since many dialects
   * implement this variant of the method. However, overriding {@link #buildInsertStatement(TableId,
   * Collection, Collection, TableDefinition)} is suggested.
   *
   * @param table the identifier of the table; may not be null
   * @param keyColumns the identifiers of the columns in the primary/unique key; may not be null but
   *     may be empty
   ...
   * Build the INSERT prepared statement expression for the given table and its columns.
   *
   * <p>By default this method calls {@link #buildInsertStatement(TableId, Collection, Collection)}
   * to maintain backward compatibility with older versions. Subclasses that override this method do
   * not need to override {@link #buildInsertStatement(TableId, Collection, Collection)}.
   *
   * @param table the identifier of the table; may not be null
   * @param keyColumns the identifiers of the columns in the primary/unique key; may not be null but
   *     may be empty
   ...
   *
   * <p>This method is only called by the default implementation of {@link
   * #buildUpdateStatement(TableId, Collection, Collection, TableDefinition)}, since many dialects
   * implement this variant of the method. However, overriding {@link #buildUpdateStatement(TableId,
   * Collection, Collection, TableDefinition)} is suggested.
   *
   * @param table the identifier of the table; may not be null
   * @param keyColumns the identifiers of the columns in the primary/unique key; may not be null but
   ...
   * for each key column should also appear in the WHERE clause of the statement.
   *
   * <p>By default this method calls {@link #buildUpdateStatement(TableId, Collection, Collection)}
   * to maintain backward compatibility with older versions. Subclasses that override this method do
   * not need to override {@link #buildUpdateStatement(TableId, Collection, Collection)}.
   *
   * @param table the identifier of the table; may not be null
   * @param keyColumns the identifiers of the columns in the primary/unique key; may not be null but
   *     may be empty
   ...
   *
   * <p>This method is only called by the default implementation of {@link
   * #buildUpsertQueryStatement(TableId, Collection, Collection, TableDefinition)}, since many
   * dialects implement this variant of the method. However, overriding {@link
   * #buildUpsertQueryStatement(TableId, Collection, Collection, TableDefinition)} is suggested.
   *
   * @param table the identifier of the table; may not be null
   * @param keyColumns the identifiers of the columns in the primary/unique key; may not be null but
   *     may be empty
   ...
   * also appear in the WHERE clause of the statement.
   *
   * <p>By default this method calls {@link #buildUpsertQueryStatement(TableId, Collection,
   * Collection)} to maintain backward compatibility with older versions. Subclasses that override
   * this method do not need to override {@link #buildUpsertQueryStatement(TableId, Collection,
   * Collection)}.
   *
   * @param table the identifier of the table; may not be null
   * @param keyColumns the identifiers of the columns in the primary/unique key; may not be null but
   ...
   * @see #statementBinder
   */
  void bindField(PreparedStatement statement, int index, Schema schema, Object value)
      throws SQLException;

  /**
   * Validate if dialect specific column types are compatible with connector. Sometimes JDBC treats
   * some column types in a SQL database the same (eg, MSSQL Server's DATETIME and DATETIME2 are
   * considered {@link java.sql.Time}). This function is used to handle these specifc column types.
   *
   * @param rsMetadata the result set metadata; may not be null
   * @param columns columns to check; may not be null
   * @throws ConnectException if column type not compatible with connector or if there is an error
   *     accessing the result set metadata
   */
  void validateSpecificColumnTypes(ResultSetMetaData rsMetadata, List<ColumnId> columns)
