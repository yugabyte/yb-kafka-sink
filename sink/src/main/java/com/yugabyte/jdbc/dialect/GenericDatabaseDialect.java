...
import com.yugabyte.jdbc.sink.JdbcSinkConfig;
import com.yugabyte.jdbc.sink.JdbcSinkConfig.InsertMode;
import com.yugabyte.jdbc.sink.JdbcSinkConfig.PrimaryKeyMode;
import com.yugabyte.jdbc.sink.PreparedStatementBinder;
import com.yugabyte.jdbc.sink.metadata.FieldsMetadata;
import com.yugabyte.jdbc.sink.metadata.SchemaPair;
...
import com.yugabyte.jdbc.source.TimestampIncrementingCriteria;
...
public class GenericDatabaseDialect implements DatabaseDialect {
  ...
  public static class Provider extends DatabaseDialectProvider.FixedScoreProvider {
    public Provider() {
      super(GenericDatabaseDialect.class.getSimpleName(), AVERAGE_MATCHING_SCORE);
    }
  ...
  protected GenericDatabaseDialect(AbstractConfig config, IdentifierRules defaultIdentifierRules) {
    ...
    this.jdbcUrlInfos =
        this.jdbcUrls
            .stream()
            .map(url -> DatabaseDialects.extractJdbcUrlInfo(url))
            .collect(Collectors.toSet());
    if (config instanceof JdbcSinkConfig) {
      ...
      quoteSqlIdentifiers =
          QuoteMethod.get(config.getString(JdbcSinkConfig.QUOTE_SQL_IDENTIFIERS_CONFIG));
    } else {
      ...
      quoteSqlIdentifiers =
          QuoteMethod.get(config.getString(JdbcSourceConnectorConfig.QUOTE_SQL_IDENTIFIERS_CONFIG));
    }
  ...
  public Connection getConnection() throws SQLException {
    ...
    List<String> urls = jdbcUrls.stream().collect(Collectors.toList());
    Collections.shuffle(urls);
    String jdbcUrl = urls.get(0);
  ...
  /**
   * Return a query that can be used to check the validity of an existing database connection when
   * the JDBC driver does not support JDBC 4. By default this returns {@code SELECT 1}, but
   * subclasses should override this when a different query should be used.
   *
   ...
   *
   * <p>By default this method adds any {@code connection.*} properties (except those predefined by
   * the connector's ConfigDef, such as {@code connection.url}, {@code connection.user}, {@code
   * connection.password}, {@code connection.attempts}, etc.) only after removing the {@code
   * connection.} prefix. This allows users to add any additional DBMS-specific properties for the
   * database to the connector configuration by prepending the DBMS-specific properties with the
   * {@code connection.} prefix.
   *
   * <p>Subclasses that don't wish to support this behavior can override this method without calling
   * this super method.
   *
   * @param properties the properties that will be passed to the {@link DriverManager}'s {@link
   *     DriverManager#getConnection(String, Properties) getConnection(...) method}; never null
  ...
  protected Properties addConnectionProperties(Properties properties) {
    ...
    config
        .originalsWithPrefix(JdbcSourceConnectorConfig.CONNECTION_PREFIX)
        .forEach(
            (k, v) -> {
   ...
   * the {@link JdbcSourceConnectorConfig#BATCH_MAX_ROWS_CONFIG batch size} of the connector. This
   * will provide a hint to the JDBC driver as to the number of rows to fetch from the database in
   * an attempt to limit memory usage when reading from large tables. Driver implementations often
   * require further configuration to make use of the fetch size.
   *
  ...
  public ExpressionBuilder expressionBuilder() {
    return identifierRules().expressionBuilder().setQuoteIdentifiers(quoteSqlIdentifiers);
  }
   ...
   */
  @Override
  public Timestamp currentTimeOnDB(Connection conn, Calendar cal)
  ...
  public Map<ColumnId, ColumnDefinition> describeColumns(
      Connection connection, String tablePattern, String columnPattern) throws SQLException {
  ...
  public Map<ColumnId, ColumnDefinition> describeColumns(
    ...
    final Set<ColumnId> pkColumns =
        primaryKeyColumns(connection, catalogPattern, schemaPattern, tablePattern);
    Map<ColumnId, ColumnDefinition> results = new HashMap<>();
    try (ResultSet rs =
        connection
            .getMetaData()
            .getColumns(catalogPattern, schemaPattern, tablePattern, columnPattern)) {
  ...
  protected Set<ColumnId> primaryKeyColumns(
      Connection connection, String catalogPattern, String schemaPattern, String tablePattern)
      throws SQLException {

    // Get the primary keys of the table(s) ...
    final Set<ColumnId> pkColumns = new HashSet<>();
    try (ResultSet rs =
        connection.getMetaData().getPrimaryKeys(catalogPattern, schemaPattern, tablePattern)) {
      while (rs.next()) {
        String catalogName = rs.getString(1);
        String schemaName = rs.getString(2);
        String tableName = rs.getString(3);
  ...
  @Override
  public Map<ColumnId, ColumnDefinition> describeColumnsByQuerying(Connection db, TableId tableId)
      throws SQLException {
  ...
  public TableDefinition describeTable(Connection connection, TableId tableId) throws SQLException {
    Map<ColumnId, ColumnDefinition> columnDefns =
        describeColumns(
            connection, tableId.catalogName(), tableId.schemaName(), tableId.tableName(), null);
  ...
  protected TableType tableTypeFor(Connection connection, TableId tableId) throws SQLException {
    ...
    glog.warn(
        "{} dialect did not find type for {} {}; using TABLE", this, tableTypeDisplay, tableId);
    return TableType.TABLE;
  }
   ...
   * @param precision the precision of the column for numeric values, or the length for non-numeric
   *     values
  ...
  public TimestampIncrementingCriteria criteriaFor(
      ColumnId incrementingColumn, List<ColumnId> timestampColumns) {
  ...
  public String addFieldToSchema(ColumnDefinition columnDefn, SchemaBuilder builder) {
    return addFieldToSchema(
        columnDefn, builder, fieldNameFor(columnDefn), columnDefn.type(), columnDefn.isOptional());
  }
  ...
  protected String addFieldToSchema(
    ...
    switch (sqlType) {
      ...
      case Types.BOOLEAN:
        {
          builder.field(
              fieldName, optional ? Schema.OPTIONAL_BOOLEAN_SCHEMA : Schema.BOOLEAN_SCHEMA);
          break;
        }
      ...
      case Types.REAL:
        {
          builder.field(
              fieldName, optional ? Schema.OPTIONAL_FLOAT32_SCHEMA : Schema.FLOAT32_SCHEMA);
          break;
        }
      ...
      case Types.DOUBLE:
        {
          builder.field(
              fieldName, optional ? Schema.OPTIONAL_FLOAT64_SCHEMA : Schema.FLOAT64_SCHEMA);
          break;
        }
  ...
  @Override
  public void applyDdlStatements(Connection connection, List<String> statements)
      throws SQLException {
    try (Statement statement = connection.createStatement()) {
  ...
  protected ColumnConverter columnConverterFor(
    ...
    switch (mapping.columnDefn().type()) {
      ...
      case Types.BIT:
        {
          /**
           * BIT should be either 0 or 1. TODO: Postgres handles this differently, returning a
           * string "t" or "f". See the elasticsearch-jdbc plugin for an example of how this is
           * handled
           */
      ...
      case Types.DATE:
        {
          return rs ->
              rs.getDate(
                  col, DateTimeUtils.getTimeZoneCalendar(TimeZone.getTimeZone(ZoneOffset.UTC)));
        }
  ...
  public StatementBinder statementBinder(
    ...
    return new PreparedStatementBinder(
        this, statement, pkMode, schemaPair, fieldsMetadata, insertMode);
  }

  @Override
  public void bindField(PreparedStatement statement, int index, Schema schema, Object value)
      throws SQLException {
    if (value == null) {
  ...
  /**
   * Dialects not supporting `setObject(index, null)` can override this method to provide a specific
   * sqlType, as per the JDBC documentation
  ...
  public List<String> buildAlterTable(TableId table, Collection<SinkRecordField> fields) {
    ...
    final ExpressionBuilder.Transform<SinkRecordField> transform =
        (builder, field) -> {
          if (newlines) {
  ...
  @Override
  public void validateSpecificColumnTypes(ResultSetMetaData rsMetadata, List<ColumnId> columns)
  ...
  protected void writeColumnsSpec(ExpressionBuilder builder, Collection<SinkRecordField> fields) {
    ExpressionBuilder.Transform<SinkRecordField> transform =
        (b, field) -> {
  ...
  public String identifier() {
    return name()
        + " database "
        + jdbcUrls.stream().map(jdbcUrl -> sanitizedUrl(jdbcUrl)).collect(Collectors.joining("::"));
  }
