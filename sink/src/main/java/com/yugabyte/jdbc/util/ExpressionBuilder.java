...
import com.yugabyte.jdbc.dialect.DropOptions;

/**
 * A class that can be used to build SQL expressions. A builder can be created with {@link
 * IdentifierRules} that dictate the quote characters and identifier delimiter, or it can be
 * obtained directly from a {@link DatabaseDialect} via the {@link
 * DatabaseDialect#expressionBuilder()} method.
 *
 ...
 * </pre>
 *
 * Note how the {@link TableId} elements are properly quoted using the {@link IdentifierRules} that
 * are passed to the builder's constructor.
 *
 * <p>This class is often used within a {@link DatabaseDialect} implementation to construct the
 * various select, insert, update, upsert, and delete statements without having to explicitly deal
 * with fully-qualified identifiers, quoting rules, sizes of lists, etc.
 */
public class ExpressionBuilder {

  /**
   * A functional interface for anything that can be appended to an expression builder. This makes
   * use of double-dispatch to allow implementations to customize the behavior, yet have callers not
   * care about the differences in behavior.
   */
  @FunctionalInterface
  ...
  /**
   * A functional interface for a transformation that an expression builder might use when appending
   ...
   * allows a caller to easily define a custom delimiter to be used between items in the list, an
   * optional transformation that should be applied to each item in the list, and the items in the
   * list. This is very handle when the number of items is not known a priori.
   *
  ...
  public interface ListBuilder<T> {
    ...
    /**
     * Define the delimiter to appear between items in the list. If not specified, a comma is used
   ...
   *
   * @param method the quoting method; may be null if the default method ({@link QuoteMethod#ALWAYS
  ...
  /**
   * Return a new ExpressionBuilder that escapes quotes with the specified prefix. This builder
   * remains unaffected.
   *
  ...
  /**
   * Append to this builder's expression the delimiter defined by this builder's {@link
   * IdentifierRules}.
   *
   * @return this builder to enable methods to be chained; never null
   */
   ...
   * Use {@link #appendIdentifier(String, QuoteMethod)} for identifiers, {@link
   * #appendColumnName(String, QuoteMethod)} for column names, or {@link #appendTableName(String,
   * QuoteMethod)} for table names.
   *
  ...
  /**
   * Append to this builder's expression the specified Column identifier, possibly surrounded by the
   * leading and trailing quotes based upon {@link #setQuoteIdentifiers(QuoteMethod)}.
   *
   * @param name the name to be appended
   * @return this builder to enable methods to be chained; never null
   */
  ...
  /**
   * Append to this builder's expression the binary value as a hex string, prefixed and suffixed by
   ...
   * then this builder delegates to the object's {@link Expressable#appendTo(ExpressionBuilder,
   * boolean)} method. Otherwise, the string representation of the object is appended to the
   * expression.
   *
  ...
  /**
   * Append to this builder's expression the specified object surrounded by quotes. If the object is
   * {@link Expressable}, then this builder delegates to the object's {@link
   * Expressable#appendTo(ExpressionBuilder, boolean)} method. Otherwise, the string representation
   * of the object is appended to the expression.
   *
   * @param obj the object to be appended
   * @return this builder to enable methods to be chained; never null
   */
