...
/**
 * This is a showcase on how to test the synchronization of the contents of a Postgres database
 * table via Confluent Kafka ecosystem with the help of Docker containers (via <a
 * href="https://www.testcontainers.org/>testcontainers</a> library).
 *
 * <p>For the test environment the following containers will be started:
 *
 * <ul>
 *   <li>Apache Zookeeper
 *   <li>Apache Kafka
 *   <li>Confluent Schema Registry
 *   <li>Confluent Kafka Connect
 *   <li>PostgreSQL
 * </ul>
 *
 * <p>Once the test environment is started, a kafka-connect-jdbc connector will be registered for
 * the <code>bookmarks</code> PostgreSQL table. The kafka-connect-jdbc connector will then poll the
 * <code>bookmarks</code> table and will sync its contents towards the <code>findinpath.bookmarks
 * </code> Apache Kafka topic.
 *
 * <p>The {@link #demo()} test will verify whether the dynamically inserted contents into the <code>
 * bookmarks</code> Postgres table get successfully synced in AVRO format on the Apache Kafka topic
 * <code>findinpath.bookmarks</code> in the same order as they were inserted.
 */
public class KafkaConnectDemoTest {
  ...
  @Test
  public void testYugabyteDBKafkaSource() {

    /*ConnectorConfiguration bookmarksTableConnectorConfig = createBookmarksTableConnectorConfig(KAFKA_CONNECT_CONNECTOR_NAME,
            POSTGRES_INTERNAL_CONNECTION_URL,
            POSTGRES_DB_USERNAME,
            POSTGRES_DB_PASSWORD);
    registerBookmarksTableConnector(bookmarksTableConnectorConfig);

    Bookmark findinpathBookmark = new Bookmark();
    findinpathBookmark.setName("findinpath");
    findinpathBookmark.setUrl("https://www.findinpath.com");
    long findinpathBookmarkId = insertBookmark(findinpathBookmark);

    Bookmark twitterBookmark = new Bookmark();
    twitterBookmark.setName("twitter");
    twitterBookmark.setUrl("https://www.twitter.com");
    long twitterBookmarkId = insertBookmark(twitterBookmark);

    awaitForTopicCreation(KAFKA_CONNECT_OUTPUT_TOPIC, 10, TimeUnit.SECONDS);


    List<ConsumerRecord<String, GenericRecord>> genericRecords = dumpGenericRecordTopic(KAFKA_CONNECT_OUTPUT_TOPIC, 2, POLL_TIMEOUT_MS);
  ...
  private Connection connect() throws SQLException {
    return DriverManager.getConnection(
        postgreSQLContainer.getJdbcUrl(), POSTGRES_DB_USERNAME, POSTGRES_DB_PASSWORD);
  }
  ...
  private static ConnectorConfiguration createBookmarksTableConnectorConfig(
